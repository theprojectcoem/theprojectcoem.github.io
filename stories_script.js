...
async function fetchcuentoJSON(cuento) {
    try {
        ...
        const response = await fetch(`static/Cuentos_english/${cuento}.json`);
...
async function fetchaudiosource(cuento) {
    let audioUrl = `/static/audios_en/${cuento}.mp3`;
    try {
    ...
    fetchcuentoJSON(storyid)
        .then(cuentoTextData => {
            ...
            if (cuentoTextData && cuentoTextData.text) {
                ...
                cuentoTextContainer.textContent = "No story found.";
            }
        })
        .catch(error => {
...
function updateTopStories(node, gData) {
    ...
    if (node.neighbors && Array.isArray(node.neighbors) && node.neighbors.length > 0) {
        ...
        console.log("Node neighbors are not defined or not an array. Adding a random neighbor.");
